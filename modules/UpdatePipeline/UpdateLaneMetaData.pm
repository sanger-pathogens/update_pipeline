...
sub _differences_between_file_and_lane_meta_data
{
  ...
  elsif( defined($self->file_meta_data->total_reads ) && $self->file_meta_data->total_reads > 10000 && $self->lane_meta_data->{lane_processed} > 0 &&   !( $self->file_meta_data->total_reads >= $self->lane_meta_data->{total_reads}*0.98  && $self->file_meta_data->total_reads <= $self->lane_meta_data->{total_reads}*1.02 ) )
  {
