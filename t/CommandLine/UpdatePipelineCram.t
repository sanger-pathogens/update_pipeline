...
BEGIN {
    ...
    my @study_file_locations = ("/seq/2442/2442_5.cram",);
...
is("2442_5.cram",$vrtrack->{_dbh}->selectrow_arrayref('select name from latest_file where name like "2442_5.cram"')->[0],'File name added');
is("/path/to/reference",$vrtrack->{_dbh}->selectrow_arrayref('select reference from latest_file where name like "2442_5.cram"')->[0],'File reference added');
is(6,$vrtrack->{_dbh}->selectrow_arrayref('select type from latest_file where name like "2442_5.cram"')->[0],'File with correct type added');
...
sub delete_inserted_vrtrack_test_data
{
  ...
  $vrtrack->{_dbh}->do('delete from file where name in ("2442_5.cram")');
}
